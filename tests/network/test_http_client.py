...
def test_successful_post(mock_response):
    with patch("sync_airbnb.network.http_client.curl_requests.post", return_value=mock_response):
...
def test_auth_401():
    ...
    with patch("sync_airbnb.network.http_client.curl_requests.post", return_value=mock):
...
def test_retryable_503():
    ...
    with patch("sync_airbnb.network.http_client.curl_requests.post", return_value=mock):
        with pytest.raises(Exception) as e:  # backoff raises Exception for retryable errors
